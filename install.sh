...
DOWNLOAD_URL=$(curl -fsSL https://api.github.com/repos/APP_REPO/releases/latest \
        | grep browser_download_url \
        | cut -d '"' -f 4)
WHEEL_DIR=$(mktemp -d 2>/dev/null)
WHEEL_FILE="$WHEEL_DIR/${DOWNLOAD_URL##*/}"
trap "rm -rf $WHEEL_DIR >/dev/null 2>&1 || true && echo \"ERROR: APP_NAME wasn't installed\" && exit 1" ERR
